...
const sideMenu = Markup.keyboard([
...
// === /start — приветствие с фото и боковыми кнопками ===
bot.start((ctx) => {
  ctx.session = {};
...
Нажмите кнопку "📥 Показать меню", чтобы начать`,
      parse_mode: "Markdown",
      reply_markup: sideMenu.reply_markup,
    }
  );
});

// === Управление меню ===
bot.hears("📥 Показать меню", (ctx) => {
  return ctx.reply("📋 Главное меню открыто:", mainMenu);
});

bot.hears("📤 Скрыть меню", (ctx) => {
  return ctx.reply(
    "Меню скрыто. Чтобы открыть снова — нажмите 📥 Показать меню",
...
bot.on("text", async (ctx) => {
  ...
  if (ctx.session.mode === "calc") {
    ...
    await ctx.reply(
      `📦 Сумма: ${amount} €\n` +
        ...
        `⚠️ Для крупногабаритных или тяжёлых заказов возможна доплата 11€/кг.`,
      mainMenu
    );
  ...
  return ctx.reply("Выберите действие из меню", sideMenu);
});
